...
若我们希望增加软绳的硬度，例如使其能抗弯折，在外观上更像钢绳，可以额外用一些弹簧，将每个质点的与相隔的邻居相连，这样就可以用弹簧实现抗弯折的力约束：

```cpp
...//软绳代码
for (int i = 0; i < std::size(points) - 2; i++)
  AddSpring(particles[i], particles[i + 2], 2, 1);
```

### 模拟布料

布料的弹簧结构与钢绳类似，只不过由一维变成了二维，也因此需要对斜方向也进行约束，综合来说，布料需要的弹簧结构，可以根据其作用分为 3 类：
...
for (int y = 0; y < n; y++)
    for (int x = 0; x < n; x++)
        points[y][x] = AddParticle((x, y));
for (int y = 0; y < size; y++)
    for (int x = 0; x < size; x++)
    {
        //抗拉伸结构
        if (x + 1 < size)
            AddSpring(points[y][x], points[y][x + 1], 1, 1);
        if (y + 1 < size)
            AddSpring(points[y][x], points[y + 1][x], 1, 1);
        //抗斜向拉伸结构
        if (x + 1 < size && y + 1 < size) //右下
            AddSpring(points[y][x], points[y + 1][x + 1], 1.41, 1);
        if (x + 1 < size && y - 1 >= 0) //右上
            AddSpring(points[y][x], points[y - 1][x + 1], 1.41, 1);
        //抗弯折弹簧
        if (x + 2 < size)
            AddSpring(points[y][x], points[y][x + 2], 2, 1);
        if (y + 2 < size)
            AddSpring(points[y][x], points[y + 2][x], 2, 1);
    }
```

## 实际实现

理解了上述质点弹簧的基本原理和制作思路，我们就能很容易的将其转换到其他软件中实现了。例如我们可以将其移植到 Unity，同时利用 Unity 提供的很多特别功能，针对性的开发，从而实现一个更切合使用环境和更高性能的质点弹簧系统。

1. 使用 Unity 的 ECS 框架实现质点弹簧

   ECS 框架，非常适合大批量处理小型物体，其完美符合计算机局部性原理，所以运行效率很高，而且

...
*为啥是袜子而不是裤子，首先我们没法获取到袜子模型（腿部模型只有一层，看成裤子就没袜子了），另外从游戏内看裤子模型比袜子模型更复杂，大部分源模型的特征也更接近袜子（穿裙子后谁还穿正经裤子啊，结果不是黑丝就是白丝）*。
...
2. 裙子背面根据源材质情况，用的半透明材质。没有编写pmat文件，因为虽然面积较大，但有不透明的裙子正面做遮挡。

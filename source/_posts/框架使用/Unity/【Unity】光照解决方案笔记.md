...
- 光照贴图：存储模型表面收到的光照信息，如光颜色强度、光入射方向、灯光阴影信息。
- 光照探针：利用球谐函数存储的某空间位置收到周围环境的低频漫射光信息，类似轻量级IBL。
...
指定场景中实时光照和烘焙光照（GI）的混合方式，**仅供混合模式的灯光使用**。
...
- Shadowmask
  ...
  - 表示一种光照混合模式。
  - 表示一种静态阴影功能。
  - 表示一种额外的光照贴图，用于确定物体表面是否处于某些灯光的阴影中，最多支持4盏灯。

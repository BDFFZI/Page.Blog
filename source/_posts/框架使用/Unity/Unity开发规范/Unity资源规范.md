...
项目中大概率会使用第三方提供的功能，对于功能性插件，应放在`Plugins`文件夹（很多插件包导入时自动就收纳到 `Plugins` 文件夹了，属于一个事实标准）；对于美术资产，可以考虑放在`Artworks`文件夹（此时第三方模块可能包含各种美术资源，由于无法细分，所以应放在能兼容这些类型的最小父文件夹中）
